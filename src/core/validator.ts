...
import { DeploymentConfig, ProcessedConfig, ValidationResult, SystemDependencies } from '../types';
...
export class ValidationService {
   ...
   */
  async validateDeploymentConfig(config: DeploymentConfig): Promise<ValidationResult> {
    const errors: string[] = [];
  ...
  validateProjectName(projectName: string): ValidationResult {
    ...
    if (trimmed.length < VALIDATION_RULES.PROJECT_NAME.MIN_LENGTH) {
      errors.push(`Project name must be at least ${VALIDATION_RULES.PROJECT_NAME.MIN_LENGTH} character(s)`);
    }

    if (trimmed.length > VALIDATION_RULES.PROJECT_NAME.MAX_LENGTH) {
      errors.push(`Project name must be no more than ${VALIDATION_RULES.PROJECT_NAME.MAX_LENGTH} characters`);
    }

    if (!VALIDATION_RULES.PROJECT_NAME.ALLOWED_CHARS.test(trimmed)) {
      errors.push(ERROR_MESSAGES.INVALID_PROJECT_NAME);
    }
  ...
  validateDomainName(domainName: string): ValidationResult {
    ...
    if (trimmed.length < VALIDATION_RULES.DOMAIN_NAME.MIN_LENGTH) {
      errors.push(`Domain name must be at least ${VALIDATION_RULES.DOMAIN_NAME.MIN_LENGTH} characters`);
    }

    if (trimmed.length > VALIDATION_RULES.DOMAIN_NAME.MAX_LENGTH) {
      errors.push(`Domain name must be no more than ${VALIDATION_RULES.DOMAIN_NAME.MAX_LENGTH} characters`);
    }

    if (!VALIDATION_RULES.DOMAIN_NAME.PATTERN.test(trimmed)) {
      errors.push(ERROR_MESSAGES.INVALID_DOMAIN_NAME);
    }
  ...
  validatePortNumber(portNumber: number): ValidationResult {
    ...
    if (portNumber < VALIDATION_RULES.PORT_NUMBER.MIN || portNumber > VALIDATION_RULES.PORT_NUMBER.MAX) {
      errors.push(ERROR_MESSAGES.INVALID_PORT_NUMBER);
    }

    if (VALIDATION_RULES.PORT_NUMBER.RESERVED_PORTS.includes(portNumber as any)){
      errors.push(ERROR_MESSAGES.RESERVED_PORT);
    }
   ...
   */
  async checkPortConflicts(portNumber: number): Promise<ValidationResult> {
    const errors: string[] = [];
   ...
   */
  async validateSiteDoesNotExist(cleanProjectName: string): Promise<ValidationResult> {
    const errors: string[] = [];

    const siteExists = await this.nginxService.siteExists(cleanProjectName);
    if (siteExists) {
      errors.push(ERROR_MESSAGES.SITE_EXISTS);
    }
   ...
   */
  async processAndValidateConfig(config: DeploymentConfig): Promise<ProcessedConfig> {
    ...
    const cleanProjectName = InputSanitizer.sanitizeProjectName(config.projectName);
    const sanitizedDomain = InputSanitizer.sanitizeDomainName(config.domainName);
    const sanitizedPort = InputSanitizer.sanitizePortNumber(config.portNumber);
    const upstreamName = InputSanitizer.generateUpstreamName(config.projectName);
   ...
   */
  async validateNginxConfig(): Promise<ValidationResult> {
    const errors: string[] = [];

    const isValid = await this.systemService.checkNginxSyntax();
    if (!isValid) {
      errors.push(ERROR_MESSAGES.CONFIG_TEST_FAILED);
    }
   ...
   */
  async validatePreDeployment(config: DeploymentConfig): Promise<{
    isValid: boolean;
    errors: string[];
